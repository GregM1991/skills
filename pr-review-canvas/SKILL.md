...
5. Start or reuse the managed canvas server:
   ```bash
   CANVAS_DIR=/tmp/pr-review-canvas
   PORT=${PORT:-8432}
   ...
   Navigate the in-app browser to the printed URL.

   **Remote/SSH use:** if the user is on another machine and asks for a reachable URL, set `BIND_ADDR` to this system's Tailscale IPv4 address before running the managed-server snippet:
   ```bash
